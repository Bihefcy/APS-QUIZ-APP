    ...
    <div v-if="currentView === 'challenge'" class="space-y-4">
      ...
      <div class="flex justify-between items-center bg-gray-100 p-4 rounded-lg">
        ...
        <div class="flex items-center gap-4">
          <div class="text-xl">
            Correct: {{ correctAnswers.size }}/{{ currentGroupQuestions.length }}
          </div>
          <div class="text-2xl font-mono" :class="{'text-red-600': timeRemaining <= 10}">
      ...
      <div class="bg-white p-6 rounded-lg shadow">
        <div class="flex justify-between items-center mb-4">
          ...
          <span class="text-lg font-medium">
            Question {{ currentQuestionIndex + 1 }}/{{ currentGroupQuestions.length }}
          </span>
          <button 
            :disabled="currentQuestionIndex === currentGroupQuestions.length - 1"
            @click="navigateQuestion(1)"
            class="px-4 py-2 bg-blue-500 text-white rounded disabled:bg-gray-300"
          >
            Next
    ...
    <div v-if="currentQuestion.image" class="image-container">
      ...
      <div class="grid grid-cols-8 gap-2">
        <button
          ...
          :class="[
            'p-2 rounded',
            getQuestionStatusClass(question._id)
          ]"
        >
          {{ index + 1 }}
        </button>
    ...
    <div v-if="showResults" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
      <div class="bg-white p-6 rounded-lg max-w-md">
        ...
        <p class="text-lg mb-4">
          You answered {{ correctAnswers.size }} out of {{ currentGroupQuestions.length }} questions correctly!
        </p>
        <button 
...
const fetchQuestions = async () => {
  try {
    ...
    //createRandomGroups();
    const allQuestions = await getQuestions();
    const timeQuestions = allQuestions.filter(question => question.type === 'time-challenge');
...
const currentGroupQuestions = computed(() => {
  const groupQuestions = selectedGroup.value !== null ? questionGroups.value[selectedGroup.value] : [];
...
const toggleAnswer = () => {
  ...
  if (!showAnswer.value && currentQuestionIndex.value < currentGroupQuestions.value.length - 1) {
    navigateQuestion(1);
  }
};
